...
int main(int argc, char **argv) {
    ...
    for (size_t i = 1; i < n; ++i) {
        if (bodies[i].mass < mmin) mmin = bodies[i].mass;
        if (bodies[i].mass > mmax) mmax = bodies[i].mass;
    }
    ...
    if (!sprites) { perror("malloc"); return EXIT_FAILURE; }
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&ev)) {
            ...
            if (ev.type == SDL_KEYDOWN) {
                switch (ev.key.keysym.sym) {
                    ...
                    case SDLK_r:      // reset cam
                        cam.x = 0.0; cam.y = 2.0; cam.z = 15.0; cam.yaw = 0.0; cam.pitch = -0.15;
                        break;
                    // rotate camera
        ...
        // horizontal forward vector from yaw
        double fx =  cos(cam.yaw), fz = -sin(cam.yaw);
        double lx =  sin(cam.yaw), lz =  cos(cam.yaw); // left vector
        if (keys[SDL_SCANCODE_W]) { cam.x += fx*move_speed; cam.z += fz*move_speed; }
        if (keys[SDL_SCANCODE_S]) { cam.x -= fx*move_speed; cam.z -= fz*move_speed; }
        if (keys[SDL_SCANCODE_A]) { cam.x += lx*move_speed; cam.z += lz*move_speed; }
        if (keys[SDL_SCANCODE_D]) { cam.x -= lx*move_speed; cam.z -= lz*move_speed; }
        ...
        if (!paused) {
            ...
            if (steps > 0 && steps_done >= steps) running = false;
        }
        ...
        // Project all points and collect visible sprites
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (project_point(&cam, fx_proj, fy_proj, nearz, farz,
                              ...
                              &sx, &sy, &zcam)) {
                // cull off-screen with a generous margin
        ...
        // sort far->near so nearer points draw on top
        ...
        for (size_t k = 0; k < vis; ++k) {
            ...
            // depth-based dimming for a simple 3D cue
            double t = clampd((sprites[k].zcam - 2.0) / 80.0, 0.0, 1.0);
            Uint8 shade = (Uint8)lround(255.0 * (1.0 - 0.75*t)); // near=255, far~64
