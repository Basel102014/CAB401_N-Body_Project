...
int main(int argc, char **argv)
{
    ...
    if (!bodies) { perror("calloc bodies"); return EXIT_FAILURE; }
    ...
    if (!snaps.xyz) { perror("malloc snaps"); free(bodies); return EXIT_FAILURE; }

    double *masses = malloc(n * sizeof(double));
    if (!masses) { perror("malloc masses"); free(snaps.xyz); free(bodies); return EXIT_FAILURE; }
    ...
    if (threads > 1) {
        printf("Running in parallel mode (%zu threads)\n", threads);
        double avg_force = 0.0, avg_update = 0.0;
        run_nbody_parallel(bodies, n, steps, dt, threads, &avg_force, &avg_update);

        if (fp)
            fprintf(fp, "0,%zu,%zu,%.9f,%.9f,%.9f\n", n, threads, avg_force, avg_update, avg_force + avg_update);
    }
    else {
        ...
        printf("Total Force Time : %.8f s\n", total_force);
        printf("Total Update Time: %.8f s\n", total_update);
